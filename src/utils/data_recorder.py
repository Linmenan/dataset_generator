...
class DataRecorder:
    ...
    def save(self):
        ...
        with self._lock:
            ...
            data_snapshot = {tab: lines[:] for tab, lines in self.data.items()}

        # ---------- 2. 写磁盘（无锁） ----------
        try:
            with pd.ExcelWriter(file_path) as writer:
                for tab, lines in data_snapshot.items():
                    pd.DataFrame(lines).to_excel(writer, sheet_name=tab, index=False)
    ...
    def load(
        ...
        try:
            ...
            return True, agents_dict, file_name
